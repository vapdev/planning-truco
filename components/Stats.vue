            ...
            <div class="grid grid-cols-2 gap-x-4 gap-y-2">
                <div v-for="card in nonRepeatedCardsWithVotes" class="flex items-center gap-2" :key="card.value">
                    <div class="w-5 h-7 text-black oswald-font font-bold flex rounded-md justify-center bg-green-100">
                        <span class="text-lg flex justify-center"><span>{{ card.value == -1 ? '☕' : card.value }}</span></span>
                    </div>
...
const stats = computed(() => {
    if (userStore.players.length > 0) {
        ...
        const votesWithoutMinusOne = votes.filter(vote => vote !== -1);
        const totalVotes = votesWithoutMinusOne.length;
        const sumOfVotes = votesWithoutMinusOne.reduce((a, b) => a + b, 0);
