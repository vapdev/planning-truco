<template>
  <div class="h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white relative overflow-hidden flex flex-col">
    ...
    <header class="relative z-10 backdrop-blur-sm bg-white/5 border-b border-white/10 py-3">
      <div class="container mx-auto flex justify-between items-center px-6">
        <a href="/" class="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent hover:from-purple-300 hover:to-pink-300 transition-all duration-300">
    ...
    <div class="relative z-10 flex-1 flex items-center justify-center py-4 px-4 overflow-y-auto">
      <div class="backdrop-blur-sm bg-white/10 border border-white/20 p-4 sm:p-6 rounded-2xl shadow-2xl w-full max-w-lg animate-fade-in my-4">
        <form class="space-y-4">
          ...
          <div class="space-y-4">
            <div class="space-y-1">
              <label class="text-base font-semibold text-purple-200">{{ $t("roomName") }}</label>
              <UInput
                v-model="roomName"
                size="lg"
                variant="outline"
            ...
            <div class="space-y-1">
              <label class="text-base font-semibold text-purple-200">{{ $t("deck") }}</label>
              <USelectMenu
                v-model="baralho"
                :options="deckOptions"
                size="lg"
            ...
            <div v-if="baralho.value === 'custom'" class="space-y-1">
              <label class="text-base font-semibold text-purple-200">{{ $t("customDeck") }}</label>
              <UInput
            ...
            <div class="space-y-2">
              <label class="text-base font-semibold text-purple-200">{{ $t("preview") }}</label>
              <div class="flex gap-1.5 flex-wrap">
                <div v-for="card in selectedDeck.slice(0, 8)" :key="card.value" 
                     class="group relative">
                  <div class="w-10 h-12 sm:w-12 sm:h-14 bg-gradient-to-br from-purple-500 to-pink-500 rounded-md flex items-center justify-center cursor-pointer shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-purple-500/25">
            ...
            <div class="pt-2">
              <button
                ...
                class="group flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/20 rounded-lg hover:bg-white/10 transition-all duration-300 text-purple-200 hover:text-white text-sm"
              >
              ...
              <div v-show="mostrarOpcoesAvancadas" class="mt-3 space-y-3 p-3 bg-white/5 rounded-lg border border-white/10">
                <div class="flex items-center justify-between">
                  <div class="flex-1">
                    <div class="font-semibold text-purple-200 text-sm">{{ $t("revealCardsTitle") }}</div>
          ...
          <div class="pt-3">
            <button
              ...
              class="group relative w-full inline-flex items-center justify-center px-6 py-3 text-lg font-semibold text-white transition-all duration-300 ease-out transform hover:scale-105 hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-slate-900"
            >
